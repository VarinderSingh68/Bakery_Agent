...
export const Home = () => {
  ...
  return (
    <div className="min-h-screen" data-testid="home-page">
      {/* Hero Section — staggered entrance on load, mirroring the reference
          site's sequential badge -> heading -> copy -> actions reveal. */}
      <section className="relative min-h-[700px] flex items-center overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full relative z-10">
          <div className="grid md:grid-cols-2 gap-16 items-center">
            ...
            <Reveal trigger="mount" variant="scale" delay={0.3} duration={0.9} className="relative mx-auto w-full max-w-[34rem]">
      ...
      {/* Trust strip — continuous marquee, echoing the reference site's
          scrolling logo/trust bar. */}
      <div className="bg-[#2D241E] py-4 border-y border-[#3A2E24]">
        <Marquee speed={26}>
          {trustStrip.map((item, index) => (
            <span key={item} className="flex items-center px-8 text-sm font-semibold uppercase tracking-[0.18em] text-[#F2D780] whitespace-nowrap">
              {item}
              <span className="ml-8 text-[#C25934]" aria-hidden="true">✦</span>
            </span>
          ))}
        </Marquee>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-5 gap-4">
            {categories.map((cat, index) => (
              <Reveal key={cat.name} variant="up" delay={(index % 5) * 0.06} duration={0.6} threshold={0.05}>
                <Link
                  to={`/shop?category=${encodeURIComponent(cat.name)}`}
                  className="corner-accent group block bg-white rounded-2xl p-5 border border-[#E3DCCF] hover:shadow-lg hover:border-[#C25934]/30 transition-all duration-300 hover:-translate-y-1"
                  data-testid={`category-${cat.name.toLowerCase().replace(/[ &]/g, '-')}`}
                >
                  <div className="text-center">
                    <div className="w-12 h-12 mx-auto mb-3 rounded-xl bg-gradient-to-br from-[#C25934]/10 to-[#F2D780]/20 flex items-center justify-center text-2xl transition-transform duration-300 group-hover:scale-110">
                      {cat.name === 'Cakes' && '🎂'}
                      {cat.name === 'Cupcakes' && '🧁'}
                      {cat.name === 'Pastries' && '🥐'}
                      {cat.name === 'Donuts' && '🍩'}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="grid grid-cols-1 justify-items-center gap-6 sm:grid-cols-2 lg:grid-cols-4" data-testid="featured-products">
            {featuredProducts.map((product, index) => (
              <Reveal key={product.id} variant="up" delay={(index % 4) * 0.08} duration={0.65} threshold={0.05}>
      ...
      <section className="py-24 bg-white relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 gap-20 items-center">
            ...
            <Reveal variant="right" delay={0.15} className="space-y-8">
              <div>
              ...
              <div className="flex flex-wrap gap-12 pt-4">
                <div className="text-center">
                  <div className="text-4xl font-bold text-[#C25934] font-['Playfair_Display']">
